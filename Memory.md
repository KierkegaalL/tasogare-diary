...
直近PR: #40（entries.source/adjustments）・#39（ネイティブ資格情報取得）・#38（リンク昇格ロジック）・#37（過去3ヶ月タブ）はすべて `develop` へマージ済み（#40は2026-07-11時点でオープン、他はマージ済み）。

### 残タスク（2026-07-11 実装×設計書 整合チェックで確定。#1/#3はPR #40で解消）

**新規発見（docsとの矛盾・非機能要件）**
- 【要方針決定】設定画面に screen.md が定義する「バックアップする」行（サブ文言:機種変更・削除に備えてアカウントを保存）が実装に無い。実際の連携導線は `WebConnectScreen` 側にある → UI追加 or docs改訂の方針決定が必要（TaskList #2）
- 【要修正】constraints.md「リスト表示は仮想化」に対し、モバイル `CalendarScreen.tsx` の過去日記一覧が `ScrollView`+`.map()` で全件描画、`FlatList`未使用（TaskList #4）
...
次回セッションでは、上記のうち「設定画面バックアップ行の方針決定」（ユーザー判断待ち）または「モバイル過去日記一覧の仮想化」あたりから着手するのが妥当と考えられる（ユーザーの指示待ち）。
