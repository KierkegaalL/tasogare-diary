...
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: 設計（[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)、7フェーズ分割）に沿って実装中。**Phase1/2/2.5/3-4/5 = develop マージ済み／Phase6 = PR #61（レビュー済み・指摘0件・マージはユーザーが実施予定）／Phase7（段階ロールアウト）未着手**。
