...
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: 現状はJS SDK制約内での緩和策止まり（オフライン中は保存不可・タイムアウトでエラー表示）。アプリ強制終了後も保存を維持するには本格移行が必要。大規模移行のため保留（次回以降）。
