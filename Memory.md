...
最終更新: 2026-07-12（ネイティブ資格情報取得の運用配線＝PR #45。iOS実機疎通確認で4件の詰まりを解消: ①`pod install`のAppCheckCore未モジュール化エラー→`expo-build-properties`の`useFrameworks: 'static'`、②無料Apple ID（Personal Team）でのSign in with Apple provisioning失敗→`EXPO_PUBLIC_APPLE_SIGNIN_ENABLED`フラグで既定無効化（`expo-apple-authentication`はplugins未列挙でも自動適用されるためentitlement明示削除が必要だった。クライアント側`isAvailableAsync()`がentitlement無視で常にtrueを返す問題もreviewer指摘で発見・同フラグでガード）、③実機でGoogleネイティブサインインが`RNGoogleSignin: failed to determine clientID`でクラッシュ→`GoogleService-Info.plist`不使用構成のためiOSでは`webClientId`だけでなく`iosClientId`（新規`EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`）も必須と判明、`nativeCredentialSourceInstall.ts`に追加。同PRへコミット・push済み。次はGoogle連携の実機疎通確認継続。※Cron Triggers実装は別PR #44）
