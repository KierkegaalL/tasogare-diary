...
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: **設計完了**（[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)、コード変更は未着手）。ユーザーへの確認の結果、①今回はまず設計のみ作成（実装はスコープ外）、②ネイティブ設定ファイル（`GoogleService-Info.plist`/`google-services.json`）はユーザーがFirebase Consoleから取得・配置する方針、で確定。設計上の要点: Firestoreのオフライン永続化にはネイティブAuth（`@react-native-firebase/auth`）への同時移行が必須（`firestore.rules`のuidスコープをネイティブSDKが満たすため）。最大のリスクは既存匿名ユーザーのuid継続で、Workerの既存`mintCustomToken`（QRペアリングで使用中）を再利用したブリッジ（JS SDKのIDトークンを検証→同一uidのカスタムトークンを発行→ネイティブSDKへ`signInWithCustomToken`）で解決する設計。実装は7フェーズに分割済み、次回はフェーズ1（依存追加・config plugin配線）から着手可能。**reviewerファクトチェックで発見・反映済みの重大な設計修正2件**: ①`src/services/claudeWorker/client.ts`の`getIdToken()`がJS SDK固定（`getFirebaseAuth().currentUser`）で`AuthProvider`抽象を経由しておらず、対応漏れのままだとネイティブ移行後に新規端末でAI機能・QRペアリング発行・アカウント削除が軒並み`unauthenticated`で失敗する→`AuthProvider`に`getIdToken()`を追加しプロバイダ非依存にする設計に修正。②移行ブリッジ（uid継続）を既存の`nativeAuthBootstrap.ts`と同じfire-and-forgetパターンで配線すると、`authStore.initialize()`の`provider.init()`呼び出しとレースし新しい匿名uidを発行してしまう（対策すべきバグを配線ミスで再現する）→ブリッジは`nativeFirebaseAuthProvider.init()`内で`await`し完結させる設計に修正。
2. **chatの過去エントリ「要約」補完**: 当日分の`mood`/`bodyText`はサーバ側補完済みだが、複数日にまたがる過去エントリの要約補完は`docs/api-contract.md`第10章に明記のまま未着手。
3. **週別感情推移グラフ（過去3ヶ月タブ）の実データ確認**: ロジック・テストは完了済みで、`web/.env.local`のFirebase接続も完了済みのため、あとはブラウザでの実データ確認のみ残っている。
4. **PR #52（Web `/entries`側設定画面）のクローズ理由確認**: 本セッション外でクローズされており、再着手が必要か・本当に不要になったのかユーザーに要確認。
5. （優先度低・スコープ外）`QrCameraScanner.tsx`と`web/src/components/QrScanner.tsx`のQRカメラ読取ロジック二重実装の解消、`docs/design/basic-design.md`への3クライアント構成（モバイル／モバイルのExpo Webビルド／Webダッシュボード）反映、`AccountLinkSection`の連打防止強化（`WebAccountRow`と同様のrefガード化）。
