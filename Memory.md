...
- **モバイルのWeb版（Expo Web）に連携ゲート＋設定画面の連携/ログアウト行を実装**（`feature/mobile-web-connect-gate`、未push）: 直後の「Webに`/settings`新設」（PR #52・オープン中）を実機確認したユーザーから、「対象はモバイルアプリ自体のExpo Webビルド（`expo-web`、ポート8082等）だった」と訂正を受け、`web/`側ではなく`src/`（モバイル）側にPlatform.OS==='web'限定で作り直した（`web/`・モバイルのネイティブ挙動には一切手を入れていない。PR #52はそのまま残す）。
