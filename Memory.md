...
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: **設計完了＋実装着手中**（[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)）。設計上の要点: Firestoreのオフライン永続化にはネイティブAuth（`@react-native-firebase/auth`）への同時移行が必須（`firestore.rules`のuidスコープをネイティブSDKが満たすため）。最大のリスクは既存匿名ユーザーのuid継続で、Workerの既存`mintCustomToken`（QRペアリングで使用中）を再利用したブリッジ（JS SDKのIDトークンを検証→同一uidのカスタムトークンを発行→ネイティブSDKへ`signInWithCustomToken`）で解決する設計。ネイティブ設定ファイル（`GoogleService-Info.plist`/`google-services.json`）はユーザーがFirebase Consoleから取得・配置する方針で確定。実装は7フェーズに分割（`TaskCreate`済み）。
   - **✅ Phase2（Worker `/migrateToNativeAuth`）＋Phase2.5（`getIdToken`非依存化）完了**（`feature/native-firebase-migration`・**PR #56**・reviewer指摘0件）: 実機不要でユニットテスト可能な土台。`worker/src/migration.ts`（`handleMigrateToNativeAuth`＝uid検証済み前提で`mintCustomToken`を呼ぶ薄い実装）＋`/migrateToNativeAuth`ルート＋テスト、`docs/api-contract.md`§5.5。`AuthProvider.getIdToken()`を必須追加し`claudeWorker/client.ts`を`getFirebaseAuth().currentUser`直参照から`getAuthProvider().getIdToken()`経由へ変更（設計書第6章の重大指摘①を解消。`localAuthProvider`は`throw`、未サインイン→`unauthenticated`の従来挙動維持）。worker158/RN159テスト通過。
   - **残フェーズ（`@react-native-firebase/*`追加と実機検証が必要）**: Phase1（依存追加・`app.config.ts` config plugin・`.env.example`/`environments.md`にフラグ`EXPO_PUBLIC_USE_NATIVE_FIREBASE`とplist配置手順追記）／Phase3-4（`nativeFirebaseAuthProvider.ts`＝匿名認証＋移行ブリッジ。ブリッジは`init()`内で`await`完結＝fire-and-forget禁止。設計書第6章の重大指摘②）／Phase5（Apple/Googleリンク昇格）／Phase6（`nativeFirestoreEntriesRepository.ts`/`nativeFirestoreMessagesRepository.ts`＋`repository/index.ts`分岐）。**機内モードでの書込→復帰後自動同期・既存uid維持の実機確認はユーザー側必須**（§8）。
