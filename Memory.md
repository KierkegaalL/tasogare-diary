...
### 現時点（2026-07-13）で残っている実質的なタスク
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: 設計（[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)、7フェーズ分割）に沿って実装中。**Phase2/2.5・Phase1 完了（いずれも develop へ未マージのPRとしてレビュー中）／Phase3-4・5・6 未着手**。
   - **✅ Phase2（Worker `/migrateToNativeAuth`）＋Phase2.5（`getIdToken`のプロバイダ非依存化）= PR #56**（レビュー中）: `worker/src/migration.ts`新設（`mintCustomToken`を再利用し確立済みuidに同一uidのカスタムトークンを発行）＋ルート追加＋テスト、`docs/api-contract.md` §5.5追記。`AuthProvider`に`getIdToken()`を追加（`firebaseAuthProvider`＝`currentUser.getIdToken()`／`localAuthProvider`＝throw）、`claudeWorker/client.ts`を`getFirebaseAuth().currentUser`直参照から`getAuthProvider().getIdToken()`経由へ変更（移行後の新規端末でAI機能・QR発行・アカウント削除が`unauthenticated`で全滅する問題を予防）。worker158件/RN159件テスト・reviewer0件。
   - **✅ Phase1（依存追加・config plugin配線）= PR #57**（レビュー中・base develop）: `@react-native-firebase/app`・`/auth`・`/firestore` v25.1.0 追加。`app.config.ts`が`EXPO_PUBLIC_USE_NATIVE_FIREBASE`（`1`/`true`）有効時のみ`@react-native-firebase/app`+`/auth`のconfig pluginと`ios/android.googleServicesFile`を注入（`firestore`はconfig plugin無し／パスは`EXPO_PUBLIC_GOOGLE_SERVICES_PLIST`・`_JSON`で上書き可・既定ルート直下）。`.gitignore`に`GoogleService-Info.plist`/`google-services.json`追加、`.env.example`/`environments.md`に手順追記。`expo config`でフラグOFF＝非注入・ON＝注入を確認、typecheck/lint/RN156件クリーン、reviewer重大指摘なし。**⚠️ 実機検証がユーザー側に残る**: 開発ビルドで`useFrameworks:'static'`×`@react-native-firebase`のpod installが通るか（移行計画書第9章。通らなければ`$RNFirebaseAsStaticFramework`等をconfig plugin経由で注入）、ネイティブ設定ファイルの取得・配置。
   - **残フェーズ（未着手・`@react-native-firebase`実コード＋実機検証が必須）**: Phase3-4=`nativeFirebaseAuthProvider.ts`（匿名認証＋`getIdToken`＋移行ブリッジを`init()`内で`await`完結／`getAuthProvider()`にネイティブ分岐）・既存uid継続の実機確認（最重要）。Phase5=Apple/Googleリンク昇格（`nativeCredentialSource.ts`のDIシームを再利用）。Phase6=`nativeFirestore{Entries,Messages}Repository.ts`＋`repository/index.ts`分岐・機内モードでの書込→復帰後同期の実機確認。
   - 設計背景（要点）: Firestoreのオフライン永続化にはネイティブAuth（`@react-native-firebase/auth`）への同時移行が必須（`firestore.rules`のuidスコープをネイティブSDKが満たすため）。最大リスクは既存匿名ユーザーのuid継続で、Workerの`mintCustomToken`を再利用したブリッジ（JS SDKのIDトークン検証→同一uidのカスタムトークン発行→ネイティブSDKへ`signInWithCustomToken`）で解決する。**⚠️ 注意**: 本節はPR #56/#57（未マージ）を反映済みのため、`develop`にこの2PRがマージされるまでの間、同2PR側で更新したMemory.mdと本節が競合しうる（競合時はより新しい本節＝両PR反映済みを採用）。
