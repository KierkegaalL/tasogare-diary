...
最終更新: 2026-07-14（**Firestoreオフライン永続化のネイティブ移行を実装中**。設計書[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)を7フェーズにTaskCreate分解。**Phase2/2.5＝PR #56（Worker `/migrateToNativeAuth` 移行ブリッジ＋`AuthProvider.getIdToken`のプロバイダ非依存化）はdevelopへマージ済み**（reviewer指摘0件・worker158/RN159テスト通過）。**Phase1＝PR #57（依存追加＋`app.config.ts`のconfig plugin配線。`EXPO_PUBLIC_USE_NATIVE_FIREBASE`フラグ有効時のみ`@react-native-firebase/app`+`/auth`プラグインと`googleServicesFile`を注入。未設定＝現行JS SDK経路に影響なし）はレビュー中**（本ブランチ。develop取り込み済み・Memory.md衝突解消済み。ユーザーが`GoogleService-Info.plist`/`google-services.json`を配置済み）。残るPhase3-4（`nativeFirebaseAuthProvider`＝匿名認証＋uid継続の移行ブリッジ）／Phase5（Apple/Googleリンク昇格）／Phase6（Firestoreネイティブリポジトリ）は`@react-native-firebase`実コード＋実機検証が必須で未着手。詳細は下記「完了済み作業/残タスク」1項。以下は前回までの記録↓。それ以前の主な作業: モバイルのExpo Web版（`npx expo start --web`）に連携ゲート（QRカメラ読取／コード貼り付け／Googleサインイン／ゲスト利用）と設定画面の連携/ログアウト行を実装し`develop`へマージ（PR #53）。実装当初は誤って別プロジェクト`web/`側に作ってしまい、ユーザー指摘で`git reset --hard`により破棄→`src/`側に作り直した経緯あり。マージ後さらにユーザーから見た目修正指摘（web-dashboardの`/connect`と同じ白枠カードにする／連携・ログアウト行を「アカウントを削除する」と同じ見た目・位置にする）を受け追加コミット。その過程でreviewerチェック用に書いたテストが実際に連打防止バグ（`useState`のstale closureで連打時に`requestWebConnect`が二重発火）を検出→`useRef`での同期ガードに修正。並行してWeb単独機能として別途進めていたPR #52（Web `/entries`側の設定画面新設）は、**不要な機能だったためユーザーによりクローズ済み**（再着手不要）。続けて、Firestoreオフライン永続化のネイティブ移行（`@react-native-firebase`）についてユーザーへスコープ確認（①まず設計のみ ②ネイティブ設定ファイルはユーザーがConsoleから取得）のうえ、[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md) を新規作成した経緯あり）
...
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: 設計（[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)、7フェーズ分割）に沿って実装中。**Phase2/2.5 = develop マージ済み／Phase1 = PR #57レビュー中／Phase3-4・5・6 未着手**。
   ...
   - **✅ Phase1（依存追加・config plugin配線）= PR #57**（レビュー中・base develop・develop取り込み済み）: `@react-native-firebase/app`・`/auth`・`/firestore` v25.1.0 追加。`app.config.ts`が`EXPO_PUBLIC_USE_NATIVE_FIREBASE`（`1`/`true`）有効時のみ`@react-native-firebase/app`+`/auth`のconfig pluginと`ios/android.googleServicesFile`を注入（`firestore`はconfig plugin無し／パスは`EXPO_PUBLIC_GOOGLE_SERVICES_PLIST`・`_JSON`で上書き可・既定ルート直下）。`.gitignore`に`GoogleService-Info.plist`/`google-services.json`追加、`.env.example`/`environments.md`に手順追記。`expo config`でフラグOFF＝非注入・ON＝注入を確認、typecheck/lint/RN156件クリーン、reviewer重大指摘なし。**ユーザーが`GoogleService-Info.plist`/`google-services.json`を配置済み**。**⚠️ 実機検証がユーザー側に残る**: 開発ビルドで`useFrameworks:'static'`×`@react-native-firebase`のpod installが通るか（移行計画書第9章。通らなければ`$RNFirebaseAsStaticFramework`等をconfig plugin経由で注入）。
