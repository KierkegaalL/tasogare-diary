...
1. **Firestoreオフライン永続化のネイティブ移行**（`@react-native-firebase`への移行）: 設計（[docs/migration-react-native-firebase.md](docs/migration-react-native-firebase.md)、7フェーズ分割）に沿って実装中。**Phase1/2/2.5/3-4 = develop マージ済み／Phase5 = PR #59（レビュー中）／Phase6 未着手**。
...
4. **[Issue #60](https://github.com/KierkegaalL/tasogare-diary/issues/60) カレンダー月表示で日曜日の欄に日付が表示されない**: Phase5実機検証中にユーザーが発見。カレンダー機能に閉じた別件のためissue化のみ・未着手。調査の手がかりは`src/utils/date.ts`の`monthGrid()`と`src/screens/calendar/CalendarScreen.tsx`のグリッド/ヘッダー整合（issue本文に詳細記載）。
