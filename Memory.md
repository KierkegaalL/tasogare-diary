...
直近PR: #39（ネイティブ資格情報取得）・#38（リンク昇格ロジック）・#37（過去3ヶ月タブ）はすべて `develop` へマージ済み（2026-07-11時点）。

### 残タスク（2026-07-11 実装×設計書 整合チェックで確定）

**新規発見（docsとの矛盾・非機能要件）**
- 【軽微】`docs/api-contract.md:182` の `generateInsight` レスポンス例 `source.model` が `"claude-sonnet-5"` のまま。実装はGemini（`gemini-3.5-flash`）→ サンプル値のみ修正
- 【要方針決定】設定画面に screen.md が定義する「バックアップする」行（サブ文言:機種変更・削除に備えてアカウントを保存）が実装に無い。実際の連携導線は `WebConnectScreen` 側にある → UI追加 or docs改訂の方針決定が必要
- 【中〜高優先度】`entries.source`/`entries.adjustments`（プロンプト版・調整履歴の追跡、data.md/api-contract.md定義）が型・保存処理に未実装。取得はできているが保存時に破棄されている
- 【要修正】constraints.md「リスト表示は仮想化」に対し、モバイル `CalendarScreen.tsx` の過去日記一覧が `ScrollView`+`.map()` で全件描画、`FlatList`未使用
...
次回セッションでは、上記のうち優先度の高い「entries.source/adjustments未実装」「設定画面バックアップ行の方針決定」あたりから着手するのが妥当と考えられる（ユーザーの指示待ち）。
