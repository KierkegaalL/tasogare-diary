...
- **コスト制御**: フリープランのサブリクエスト上限（50）を踏まえ、`CRON_MAX_USERS`（既定20）で処理ユーザー数を、既定タイプ `weekly`/`monthly`（`CRON_INSIGHT_TYPES` で変更可）で対象を絞る。1ユーザー・1タイプの失敗は握りつぶして継続し、`failed-precondition`（当該期間の日記なし）はスキップ扱い。ログは集計値のみ（uid・本文は残さない）。列挙は `__name__` 昇順の先頭から上限件のみ（日替わりローテーションなし）のため、ユーザー数が `CRON_MAX_USERS` を超えると超過分は常に事前生成されず表示時オンデマンド生成に委ねる。
...
| `CRON_MAX_USERS` | `20` | Cron 事前生成の1回あたり処理ユーザー数上限（サブリクエスト上限対策） |
