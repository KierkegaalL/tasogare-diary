...
const MAX_ARRAY_ITEMS = 50; // events・selected・history 等の配列要素数上限（超過はリクエスト自体を拒否）
const MAX_WORDS_ITEMS = 100; // words（ことば選択）の要素数上限

// LLMへ実際に渡す会話履歴の件数上限（サーバ側切り詰め）。クライアント（DetailScreen.tsx の
// HISTORY_LIMIT=6）による送信量の絞り込みだけに頼らず、Worker側でも直近N件に切り詰めることで
// クライアントの実装変更・不具合に対する多層防御とする。MAX_ARRAY_ITEMS（拒否用の上限）より
// 小さく、通常の利用（直近6往復程度）を妨げない範囲で余裕を持たせる。
const MAX_HISTORY_MESSAGES_TO_LLM = 20;
...
export async function handleChat(env: Env, llm: LlmProvider, uid: string, data: Record<string, unknown>) {
  ...
  // LLMへ渡すのは直近 MAX_HISTORY_MESSAGES_TO_LLM 件のみ（サーバ側切り詰め。上のコメント参照）。
  const recentHistory = rawHistory.slice(-MAX_HISTORY_MESSAGES_TO_LLM);
  const history = toLlmHistory(recentHistory as { role: ChatRole; text: string }[]);
