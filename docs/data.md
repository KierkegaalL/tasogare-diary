...
- **entryId（案）**: 自動ID（推奨）。`date` は別フィールドで保持し重複を許容（1日複数エントリの可否 U-11 は未確定。当面 UI は1日1件想定だがスキーマは複数許容にしておく）。
  - 案B: `entryId = date`（1日1件を強制）。UI 前提には合うが、複数許容へ広げにくい。→ 自動ID＋`date` フィールドを推奨。

### 3.3 `users/{uid}/entries/{entryId}/messages/{messageId}`
AI対話（`visual-design.html` `.chat-bubble`）。**保存要否は U-05（既定＝保存）**。
...
> 保存しない方針に倒す場合は本サブコレクションを作らず、対話はセッション内一時保持とする。プライバシー観点では非保存が安全（第7章）。実装は設定で切替可能にする（U-05）。
...
## 10. 未確定・申し送り
- **U-05**: AI対話（`messages`）の保存要否・保持期間。既定は保存だが、非保存（プライバシー優先）も設定で選べるようにするか要決定。
- **U-11**: 1日複数エントリの可否。スキーマは自動ID＋`date` で複数許容にしているが、UI/要件の確定が必要。
- **U-10**: 感情ラベルの多段階化/数値化。
