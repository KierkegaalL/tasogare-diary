...
週次/月次まとめ（`.insight-card` / `.dash-narrative`）の生成キャッシュ。**Worker（`generateInsight`）が生成・更新**（[basic-design.md](design/basic-design.md) 第4.3節 案B。当初は Functions を想定していたが Cloud Functions 不採用のため Cloudflare Workers が担う）。
