...
- **要素**: ヘッダー（「日記の一覧」＋ダッシュボードへのリンク＋設定へのリンク）／検索欄（読み込み済み範囲をキーワード絞り込み）／エントリカード（`.diary-entry`：日付＋感情チップ／本文 `.diary-full-text`／選択語タグ `.tags-used`／気づき）／無限スクロール用の読込目印／注記（Web 限定表示）。
...
- **実装メモ（Phase4・実装済み）**: `web/src/app/entries`（`EntryList`＝`.diary-entry`）。取得は `web/src/lib/entries.ts` の `fetchEntriesPage`（`date` 降順＋`startAfter` カーソルで無限スクロール。`IntersectionObserver` で末尾の目印を検知し次ページを読込む）。検索は読み込み済みエントリの本文／選択語をクライアント側でキーワード絞り込み（Firestore 全文検索非対応・本文を外部検索サービスへ送らない方針のため）。ダッシュボード（4.1）・設定（4.4）と相互リンク。**設定（4.4）への導線はこの画面のみに置く**（ダッシュボードには追加しない。ユーザー指摘）。

### 4.4 設定（Web）（`settingsView`）
- **目的**: Webセッションのアカウント管理（連携／ログアウト／削除）を1画面にまとめる。**日記一覧（4.3）側の導線としてのみ提供**（ダッシュボード（4.1）・モバイルの `SettingsScreen`（3.9）は変更しない。モバイル側は既に「Webで見る」「バックアップする」「アカウントを削除する」を提供済みで役割が重複するため、Web側に別途追加している。ユーザー指摘）。
- **要素**: ヘッダー（戻るボタン＋「設定」）／上段の行（匿名セッション時は「スマホと連携する」、連携済み（非匿名）時は「ログアウトする」。いずれも短い説明文言つき）／下段の行「アカウントを削除する」（Worker設定時のみ表示。タップで警告文＋「本当に削除する」／「キャンセル」の2段階確認）。
- **状態**: 読込中／未サインイン（`/connect` へ遷移）／匿名（連携導線を表示）／連携済み（ログアウト導線を表示）／削除確認中／削除処理中。
- **遷移**: 「連携する」→`/connect`（ページ遷移。別タブを開かない）。「ログアウト」→サインアウトし `/connect` へ自動遷移（`useAuth` の user 変化を各画面が検知）。「本当に削除する」→ Worker の `/deleteAccount` 呼び出し→サインアウト→`/connect`。戻るボタンは `router.back()`。
- **実装メモ（実装済み）**: `web/src/app/settings`。匿名判定は Firebase Auth の `user.isAnonymous`（QRペアリング・OAuthサインインいずれも非匿名になるため、この2択で「連携する/ログアウトする」を出し分けられる）。削除は `web/src/lib/account.ts`（モバイルの `src/services/account.ts` と同じ Worker エンドポイント `/deleteAccount` を ID トークン付きで呼ぶ）。**`/connect`・`/dashboard`・モバイルには変更を加えていない**。
