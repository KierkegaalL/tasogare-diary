...
- **遷移**: 「Webで見る」→WebConnect。バックアップ→**Apple/Google アカウント連携で担保（U-13決定）**。画面上のアクションは再認証確認程度に留まる想定。
