...
- **`chat`/`chatOpening` のサーバ側文脈補完（実装済み・一部）**: 当該エントリの `mood`/`bodyText` は `entryId` からサーバ側（`getEntry`）で都度補完する。`chat` は `system` プロンプトへ注入（クライアントの `history` 切り詰めに影響されない）、`chatOpening` は `userText` へ注入する（3.4節参照。取得経路は両者で統一済み）。**未対応（将来）**: 関連する過去エントリの**要約**補完（他日の傾向等）は未実装。ストリーミングは未採用（非ストリーミング＋`maxOutputTokens` 上限）。
