...
- 案: まとめ生成のみ上位（Opus 4.8 `claude-opus-4-8`）を用いる選択肢もあるが、コストと品質のバランスで Sonnet 5 を既定推奨。モデルは環境変数で差し替え可能にする。
...
## 10. 未確定・申し送り
- **U-12**: モデル選定の最終確定とコスト上限（第1.3節は推奨）。
- **U-05**: 対話履歴の保存要否（`chat` の `history` 保存 or セッション限定）。
- **U-06**: 連想/候補の生成主体（`suggestWords` に集約するか、固定辞書併用か）。
- **プロンプト設計の詳細**（system/few-shot、`promptVersion` 運用）は Notion 要件（プロンプト設計方針）と本書を突き合わせて確定。
- **感情推定の閾値・自由語→enum 写像**の具体規則（[data.md](data.md) 第4章と連動）。
- **ストリーミング**採否（対話/生成の体感改善）。
- **モデルの環境変数化**: 第1.3節でモデルを環境変数で差し替える方針としたため、[environments.md](../.claude/rules/environments.md) に環境別のモデル設定項目を追記する（現状はキー切替のみ規定）。
