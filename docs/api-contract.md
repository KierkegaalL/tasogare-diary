...
- Web は `signInWithCustomToken(customToken)` でサインイン。**実装済み**（Web 側 `web/src/lib/pairing.ts`。`web/src/app/pair`＝QR ディープリンク着地、`web/src/app/connect`＝コード貼り付け導線）。カメラでの QR ライブ読取は後続（[web/README.md](../web/README.md)）。
