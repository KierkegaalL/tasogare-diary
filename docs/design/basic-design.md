...
### 2.1 構成要素の位置づけ
> **クライアントは3種類**（詳細設計段階で確定。[architecture.md](../architecture.md) 第4/6章参照）。「モバイル」と「Web ダッシュボード」の2区分に見えるが、実際には**同一コードベース（`src/`・React Native/Expo）から書き出される2つのビルド成果物**（ネイティブアプリ／Expo Webビルド）と、**別プロジェクト（`web/`・Next.js）の Web ダッシュボード**を合わせた3クライアントで構成される。

| 層 | 実体 | 役割 |
|---|---|---|
| モバイル（ネイティブ） | React Native / Expo（Notion `プラットフォーム`）。iOS/Android ネイティブビルド | 日々使う companion 体験。日記作成4ステップ、ホーム、カレンダー/一覧、詳細＋AI対話、設定、QR表示 |
| モバイルの Expo Web ビルド | `src/` と同一コードベースを `npx expo start --web` 等でブラウザ向けに書き出したもの（`Platform.OS === 'web'`） | ネイティブと同じ日記機能に加え、**このブラウザをスマホの日記と連携させるゲート**（`WebConnectGate`）のみ追加で持つ。QRカメラ読取／コード貼り付け／Googleサインイン／ゲスト利用の4導線（[architecture.md](../architecture.md) 第4.2節。第3.4節のQRペアリングとは逆方向・別プロジェクトのフローのため混同しないこと） |
| Web ダッシュボード | 別プロジェクト（`web/`・Next.js）、ブラウザ（`tasogare-diary.app/dashboard`・`/connect`） | **分析・振り返り専用**（閲覧のみ、U-09）。感情推移・よく使う言葉・月次まとめ、デバイス連携（QR読み取り側） |
...
> **モバイルの Expo Web ビルドと Web ダッシュボードを混同しないこと**: 前者は「スマホアプリ自体をブラウザで開いたもの」で日記の読み書きが可能な companion 体験そのもの、後者は「分析専用の別アプリ」で書き込み機能を持たない（U-09）。両者はいずれもブラウザで動くため見た目が近く、実装時に一度混同が発生した経緯がある（[architecture.md](../architecture.md) 第4.2節）。

### 2.2 構成図

```mermaid
graph TD
  subgraph Mobile["モバイル・ネイティブ (React Native / Expo)"]
    M1["日記作成4ステップ<br/>きもち→できごと→ことば→たしかめる"]
    M2["ホーム / カレンダー・一覧 / 詳細+AI対話"]
    M3["設定 / Webで見る（QR表示）"]
  end
  subgraph MobileWeb["モバイルの Expo Web ビルド<br/>（src/ と同一コードベース）"]
    MW1["Mobile と同じ日記機能"]
    MW2["WebConnectGate<br/>（このブラウザをスマホの日記と連携）"]
  end
  subgraph Web["Web ダッシュボード（別プロジェクト web/）"]
...
### 2.3 モバイルと Web の役割分担
- **モバイル（ネイティブ／Expo Webビルド共通）**: 入力と日々の伴走。編集・生成・保存・軽い振り返り・AI対話。両ビルドは同一コードベース（`src/`）から生成されるため、日記機能自体に差はない。
- **モバイルの Expo Web ビルド固有の役割**: 上記に加えて「このブラウザをスマホの日記と連携させる」ゲート（`WebConnectGate`）のみを持つ。これは Web ダッシュボードへの読み取り専用アクセスとは無関係で、あくまで companion 体験（書き込み可能）をこのブラウザでも使えるようにするための導線。
- **Web ダッシュボード（別プロジェクト `web/`）**: 読み取り中心の俯瞰。感情推移グラフ（`.mood-chart`）、よく使う言葉ランキング（`.word-rank`）、AIによる月次まとめナラティブ（`.dash-narrative`）。書き込み機能は持たせない方針（**決定・U-09**：Web ダッシュボードは閲覧専用）。
...
1. **きもち／できごと入力**: ユーザーが一言入力、または候補チップ（`.pebble`）を選択。候補チップは初期は固定＋利用傾向で差し替え（**決定・U-06**：候補提示自体は都度 Claude＋過去傾向、候補チップ初期表示は固定辞書＋傾向差し替え）。
...
- **会話履歴は保存する（U-05 決定）**。日記本文と同じセンシティブ度で uid スコープ管理。
...
- **生成主体（決定・U-07）**: **案B「Functions で集計・キャッシュし、Claude で文章化」を採用**。Web と整合し、送信データを最小化しやすい。負荷は Web 表示時のオンデマンド＋日次バッチの折衷（詳細: [api-contract.md](../api-contract.md) `generateInsight`）。
  - 不採用の代替案（参考）: 案A＝クライアント集計＋Claude で文章化（端末差・オフライン整合に難）／案C＝完全オンデマンド生成（応答遅延・API コスト増）。
...
| 日記エントリ | 1日1件（U-11 決定） | 日付、生成本文、感情ラベル、選択語群、作成/更新日時、調整履歴あり（`adjustments` フィールド、任意。[data.md](../data.md) 3.2節） |
| 選択ワード／タグ | 気持ち・できごと・連想語 | 語、カテゴリ（mood/event/assoc）、頻度 |
| 気分値 | 感情ラベル | 3段階（後述） |
| 対話（チャット） | 詳細画面の会話 | エントリID、メッセージ列（ai/me）、保存する（U-05 決定） |
...
- **モデル選定（決定・U-12）**: 連想語提案／AI対話／調整＝**Claude Haiku 4.5**、日記文生成／週次・月次まとめ＝**Claude Sonnet 5**。用途別に使い分け、環境変数で差し替え可能（詳細: [api-contract.md](../api-contract.md) 第1.3節、[environments.md](../.claude/rules/environments.md)）。
...
- **バックアップ**: 設定「バックアップする」（`.settings-row` = 「機種変更・削除に備えてアカウントを保存」）。**Apple/Google アカウント連携で担保（U-13 決定）**。詳細は [screen.md](../screen.md) 3.9節。
...
| U-04 | **解決済** | Web クライアント実装 → **Next.js 別アプリ＋Firebase Hosting**、型/トークンは `shared` パッケージで共有（[architecture.md](../architecture.md) 第6章） | 確定 |
| U-05 | **解決済** | AI対話の会話履歴 → **保存する**（`messages`、uid スコープ、日記同様センシティブ扱い） | 確定 |
| U-06 | **解決済** | 候補チップ・連想語 → **都度 Claude＋過去傾向**（`suggestWords`）。候補チップ初期は固定辞書＋傾向差し替え | 確定 |
| U-07 | **解決済** | インサイト生成 → **Functions で集計・キャッシュし Claude で文章化**（案B、[architecture.md](../architecture.md)／api-contract.md `generateInsight`） | 確定 |
| U-08 | **解決済** | QR 認証 → **短命回転トークン方式（60秒）**＋Apple/Google サインイン代替 | 確定 |
| U-09 | **解決済** | Web ダッシュボード → **閲覧専用** | 確定 |
| U-10 | **解決済（当面）** | 感情ラベル → **当面3段階固定**。多段階化/数値化は将来拡張余地として保留 | 確定（将来余地） |
| U-11 | **解決済** | 日記エントリ → **1日1件**（スキーマは自動ID＋date で複数許容のまま、UI で1件制約） | 確定 |
| U-12 | **解決済** | Claude モデル → **連想/対話/調整=Haiku 4.5、生成/まとめ=Sonnet 5**（環境変数で差し替え可）。レート/リトライは api-contract.md 第7章 | 確定 |
| U-13 | **解決済** | バックアップ → **Apple/Google アカウント連携**で担保 | 確定 |
| U-14 | **解決済** | 対応言語 → **日本語優先**、多言語化を阻害しない実装 | 確定（当面日本語） |
...
> **完了状況（2026-07-07 更新）**: 以下は本書執筆時点（詳細設計着手前）の申し送りである。その後 U-01〜U-14 はすべて確定し、詳細設計4点（[architecture.md](../architecture.md)／[data.md](../data.md)／[screen.md](../screen.md)／[api-contract.md](../api-contract.md)）も作成済み。以降の設計変更は各ドキュメントを直接更新し、要件変更は Notion を先に更新する（`CLAUDE.md` 原則3）。

- **要件の整備（進捗）**: U-01（4ステップ定義）／U-02（Notion 要件定義書の未整備）は本対応で**解決済み**（Notion「たそがれ日記 要件定義書」作成、フローを「きもち→できごと→ことば→たしかめる→灯」に統一）。残る未確定（U-03〜U-14）はすべて確定済み（詳細は各詳細設計ドキュメントを参照）。
- **`architecture.md`**（作成済み）:
  - モバイルのナビゲーション構成（React Navigation：native stack ＋ bottom tabs）と状態管理（**Zustand ＋ TanStack Query に決定**、companion 体験の軽さと学習コスト観点）。
  - Web ダッシュボードの実装方式（**Next.js 別アプリ＋Firebase Hosting に決定**、U-04）。
  - オーブアニメーションの数値仕様（`breathe` 4.8s / scale 1↔1.055、reduced-motion 分岐）と感情色の適用規則。
- **`data.md`**（作成済み）: エンティティの確定スキーマ、Firestore コレクション構造とセキュリティルール、オフライン同期戦略、U-05（対話は保存する）/U-09（Web は閲覧専用）/U-10（多段階化は将来）/U-11（1日1件）の確定。
- **`api-contract.md`**（作成済み）: Claude 各用途（連想/生成/調整/対話/まとめ）と QR発行・照合の Functions エンドポイント仕様、認証方式（U-08：短命回転トークン）、モデル・コスト（U-12：Haiku 4.5／Sonnet 5 使い分け）。
- **プライバシー詳細設計**: 削除フロー（対話・ペアリング含む `recursiveDelete`）、ログのマスキング方針、Claude 送信ペイロードの最小化仕様は `data.md`／`api-contract.md` に反映済み。
- **レビュー判断**: 本書のスコープ（基本方針・全体構造）および詳細設計は完了。ステップ4（実装）着手可能。
