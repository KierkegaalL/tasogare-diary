...
> **ステータス: フェーズ1〜7完了**（フェーズ7は配信予定未定のためスコープを縮小し、ネイティブ経路の既定推奨値化のみ対応。実際の段階ロールアウトはTestFlight/Play Store配信計画が具体化してから着手）。フェーズ単位の詳細は第7章参照。
>
...
- Web版連携ゲート（`WebConnectGate.tsx`）・`webOAuth.ts`、および `pairing.ts` のうち **Web 連携ゲートが使う部分**（`signInWithPairingToken`/`extractPairingToken`）は Web 専用機能であり、**この移行の対象外**（Web は今後も Firebase JS SDK のまま）。ただし `pairing.ts` の `createPairingToken()`（モバイル側のQR発行）は Worker 呼び出し（`claudeWorker/client.ts`）を経由するため対象内（詳細は第6章）。「`pairing.ts` 丸ごと対象外」ではない点に注意。
...
| `src/services/auth/types.ts` | `AuthProvider` に `getIdToken(): Promise<string>` を追加（下記 `claudeWorker/client.ts` 対応のため）。`AuthUser` はそのまま。既存実装にも追加する: `firebaseAuthProvider` は `getFirebaseAuth().currentUser?.getIdToken()`、`nativeFirebaseAuthProvider` はネイティブSDKの `auth().currentUser?.getIdToken()`。**`localAuthProvider`（Firebase未設定時のローカル匿名）は実トークンを持たないため `getIdToken` は `throw` する**（`callClaudeWorker` は Worker 設定時＝Firebase 設定時にしか到達せず `localAuthProvider` 経由では呼ばれないため実害はないが、偽トークンを返して静かに `unauthenticated` 失敗するのを防ぐため明示的に投げる） |
...
6. **フェーズ5（実装済み・Google実機検証完了）**: Apple/Googleリンク昇格をネイティブAuthプロバイダ側にも実装。`nativeFirebaseAuthProvider.ts`の`linkWith`に`NativeAuthBinding.getCurrentUser()`（同期）でのセッション有無・匿名判定を先に行い、`nativeCredentialSource.ts`（既存のネイティブ非依存ロジック）経由で資格情報を取得したうえで`nativeFirebaseAuthProviderInstall.ts`の`linkWithCredential`（`auth.GoogleAuthProvider.credential`/`new auth.OAuthProvider('apple.com').credential`）へ差し替える形で実装。**Apple向け`credential()`はオブジェクト引数`{idToken, rawNonce}`が正しい形式**（`@react-native-firebase/auth`の型定義＝`namespaced.d.ts`は位置引数`(token, secret)`と宣言しているが、実装＝`lib/providers/OAuthProvider.ts`はオブジェクト引数を要求しており型定義側が不正確。当初位置引数で実装しPRレビューで発覚・修正し、正しい引数形状の型キャストを介して呼ぶよう対処し回帰テストを追加済み）。Google実機検証は成功（uid維持・Firestoreデータ維持・再起動後の復元・連携済み後のUI非表示を確認）。Apple実機検証は有料Apple Developer Program未加入のため保留。
7. **フェーズ6（実装済み・実機検証完了）**: `nativeFirestoreEntriesRepository.ts`/`nativeFirestoreMessagesRepository.ts`（`@react-native-firebase/firestore` の modular API＝`firebase/firestore` JS SDK とほぼ同じ関数シグネチャ。`getFirestore`/`collection`/`doc`/`query`/`onSnapshot`等）を実装し、既存の `firestoreEntriesRepository.ts`/`firestoreMessagesRepository.ts` とロジックを揃えた（import元のみ差し替え）。`getEntriesRepository()`/`getMessagesRepository()`（`src/services/repository/index.ts`）に `shouldUseNativeFirebase()` 分岐を追加し、Auth と同じフラグで揃ってネイティブ経路へ切り替わるようにした。オフライン永続化はネイティブSDKの既定設定（`persistence: true`）で自動的に有効になるため、追加の `settings()` 呼び出しは不要。**オフライン書込→復帰後自動同期の実機確認（iPhoneのWi-Fi設定でDNSを到達不能アドレスへ手動設定し、疑似オフライン状態を作る方式）をユーザーが実施し成功**: ①アプリ生存のまま復帰するケース、②書込が保留中の状態でアプリを完全終了→復帰後に再起動するケース、のいずれもホーム/カレンダー画面への反映とFirestore側への同期を確認済み（②はJS SDK版では復元できないケースであり、ネイティブ移行の主目的である永続キューの効果を実証）。
8. **フェーズ7（判断完了・スコープを縮小して対応）**: 当初想定していた「社内配布→一部ユーザー→全体」の段階ロールアウトは、本セッション時点でTestFlight/Play Store配信自体が未定（開発ビルドのみでの運用）のため、実施対象となるユーザーが存在しない。よって本フェーズは以下の2点に縮小して対応した。
   - **`EXPO_PUBLIC_USE_NATIVE_FIREBASE` を開発/配布ビルドの既定推奨値に変更**: Phase1〜6がすべて実装・実機検証済み（uid継続・Apple/Googleリンク昇格・オフライン同期）となったため、[.env.example](../.env.example)の値を空欄から`1`へ変更し、開発/配布ビルドでは有効化を推奨する記述に更新（[environments.md](../.claude/rules/environments.md)も同様に更新）。Expo Go/Webは引き続き未設定のままJS SDK経路を使う（`shouldUseNativeFirebase()`のコード側フォールバック=falseは変更していない。Expo Goでは`.env`を明示的に未設定にする現行運用を維持することで、既定値変更によるExpo Go起動破壊のリスクを避けた）。
   - **旧経路（JS SDK）は削除しない**: `firebaseAuthProvider.ts`/`firestoreEntriesRepository.ts`/`firestoreMessagesRepository.ts`等は、Web・Expo Goが恒久的に必要とする経路であるため撤去しない。「廃止」はモバイルのネイティブビルドにおける既定経路をネイティブ側に切り替えたことのみを指し、コード自体の削除は行わない。
   - 実際の段階ロールアウト（社内配布→一部ユーザー→全体）は、TestFlight/Play Store配信の計画が具体化した時点で改めて着手する（残タスク）。
...
- ~~（iOS詰まりどころの先読み）`useFrameworks: 'static'` との相性~~ → **解決済み**: `@react-native-firebase` と `expo-build-properties` の `{ ios: { useFrameworks: 'static' } }`（`@react-native-google-signin` 対策で設定済み）の組み合わせは、Phase1〜6の実機ビルド（`pod install`＋Xcode Run）で問題なく通ることを確認済み（追加のPodfile設定は不要だった）。
