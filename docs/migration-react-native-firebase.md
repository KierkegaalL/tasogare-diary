...
> **ステータス: フェーズ1〜6実装済み（フェーズ7＝段階ロールアウトは未着手）**。フェーズ単位の詳細は第7章参照。実機でのオフライン同期検証（フェーズ6）は未実施。
>
