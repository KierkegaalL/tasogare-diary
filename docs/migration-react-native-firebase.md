...
> **ステータス: フェーズ1〜6実装済み・実機検証済み（フェーズ7＝段階ロールアウトは未着手）**。フェーズ単位の詳細は第7章参照。
>
...
- **（iOS詰まりどころの先読み）`useFrameworks: 'static'` との相性**: `app.config.ts` は既に `@react-native-google-signin` の pod install 対策として `expo-build-properties` の `{ ios: { useFrameworks: 'static' } }` を設定済み（Memory.md「iOS 開発ビルド作成時の既知の詰まりどころ」）。`@react-native-firebase` は静的フレームワーク（`use_frameworks! :linkage => :static`）と組み合わせると pod install / ビルドで追加設定を要する場合がある（RNFirebase 側が `use_frameworks` 使用時の注意を案内している）。既に static 指定がある本プロジェクトでは、フェーズ1（開発ビルド起動確認）でこの組み合わせが通ることを最優先で確認し、通らない場合の対処（RNFirebase の指定する追加 Podfile 設定を config plugin 経由で注入する等）を早期に洗い出す。
