...
6. **フェーズ5（実装済み・実機検証待ち）**: Apple/Googleリンク昇格をネイティブAuthプロバイダ側にも実装。`nativeFirebaseAuthProvider.ts`の`linkWith`に`NativeAuthBinding.getCurrentUser()`（同期）でのセッション有無・匿名判定を先に行い、`nativeCredentialSource.ts`（既存のネイティブ非依存ロジック）経由で資格情報を取得したうえで`nativeFirebaseAuthProviderInstall.ts`の`linkWithCredential`（`auth.GoogleAuthProvider.credential`/`new auth.OAuthProvider('apple.com').credential`）へ差し替える形で実装。Apple実機検証は有料Apple Developer Program未加入のため保留、Googleのみ先行して実機検証する。
