...
- **データ永続はリポジトリ層で抽象化**（`services/repository`）。Firebase 未設定時はローカル（AsyncStorage）、設定時は Firestore（`users/{uid}/entries`）に切り替わる。`EXPO_PUBLIC_USE_NATIVE_FIREBASE=1`（ネイティブFirebase移行フラグ）有効時は、上記の代わりに `nativeFirestoreEntriesRepository`/`nativeFirestoreMessagesRepository`（`@react-native-firebase/firestore`）が使われる（詳細は [migration-react-native-firebase.md](migration-react-native-firebase.md) フェーズ6）。
- **Firestore のオフライン永続（RN 制約・JS SDK 経路）**: Firebase JS SDK は RN で IndexedDB を使えず**メモリキャッシュ中心**（`experimentalForceLongPolling` を有効化）。永続化されたローカルキューを持たないため、**オフライン中に発行した書込 Promise はオンライン復帰までハングする**（アプリの生存中はメモリ上の書込は再送されるが、プロセスが終了すれば失われる）。`constraints.md` が求める本格的なオフライン永続は、`@react-native-firebase`（ネイティブ）への移行で対応する（実装・実機検証とも完了・[migration-react-native-firebase.md](migration-react-native-firebase.md) フェーズ6。ネイティブSDKは既定でローカル永続キューを持つため、`EXPO_PUBLIC_USE_NATIVE_FIREBASE=1` 環境ではオフライン中の書込もキューされ、アプリを完全終了しても永続化されたまま復帰後に自動同期されることを実機で確認済み）。
