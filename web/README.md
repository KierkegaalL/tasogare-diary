...
| `/settings` | 設定（匿名時「スマホと連携する」／連携済み時「ログアウトする」＋アカウント削除）。`/entries` からのみ導線あり | 4.4 |
