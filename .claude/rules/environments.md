...
## ネイティブ Firebase 移行設定（Phase1〜6実装・実機検証完了・`@react-native-firebase`）

Firestore のオフライン永続キューを得るため、認証・Firestore アクセスを Firebase JS SDK からネイティブ SDK（`@react-native-firebase/app` / `/auth` / `/firestore`）へ移行済み（設計・手順の正は [docs/migration-react-native-firebase.md](../../docs/migration-react-native-firebase.md)）。既存の「ネイティブ資格情報取得」と同じく **フラグで開発/配布ビルドのみ有効化し、Expo Go / Web は現行の JS SDK 経路のまま**にする。

- **有効化フラグ**: `.env` に `EXPO_PUBLIC_USE_NATIVE_FIREBASE=1`（または `true`）。`app.config.ts` が `@react-native-firebase/app`（+ `auth`）の config plugin と下記 `googleServicesFile` を有効時のみ含める（`firestore` は config plugin を持たない）。**未設定ではプラグインを含めない**ため、ネイティブ設定ファイルが無くても prebuild は通り、現行動作に影響しない。**Expo Go では設定しないこと**。**フェーズ7（移行計画書第7章）**: 既存匿名ユーザーのuid継続・Apple/Googleリンク昇格・オフライン書込→復帰後自動同期のいずれも実機検証済みのため、**開発/配布ビルドではこのフラグを `1` にするのが既定の推奨設定**（[.env.example](../../.env.example) もこれに合わせて更新済み）。TestFlight/Play Store配信は本セッション時点で未定（開発ビルドのみで運用中）のため、社内配布→一部ユーザー→全体という段階ロールアウトの実施自体は配信開始時期が決まってから着手する。JS SDK経路（`firebaseAuthProvider.ts`/`firestoreEntriesRepository.ts`等）はWeb・Expo Go向けに撤去せず残す（廃止するのはモバイルのネイティブビルドがデフォルトでネイティブ経路を使う、という既定値の切り替えのみで、コード自体は削除しない）。
...
- **`useFrameworks: 'static'` との併用**: 本プロジェクトは既に `expo-build-properties` で iOS の静的フレームワークを指定済み。`@react-native-firebase` は静的フレームワーク併用時に追加の Podfile 設定（`$RNFirebaseAsStaticFramework` 等）を要する場合があるが、**Phase1〜6の実機ビルド（`pod install`＋Xcode Run）で問題なく通ることを確認済み**（移行計画書第9章は解決済みとして整理）。
- **実機検証**: ネイティブモジュールの動作・オフライン永続化（機内モードでの書込→復帰後同期）・既存 uid の継続（移行ブリッジ）・Apple/Googleリンク昇格（Googleのみ）は、いずれもユーザー側の開発ビルド/実機で確認済み（同第8章）。
